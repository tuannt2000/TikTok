export const GET_ALL_USER = 'GET_ALL_USER';
export const SET_ALL_USER = 'SET_ALL_USER';
export const GET_USER_FOLLOWING = 'GET_USER_FOLLOWING';
export const SET_USER_FOLLOWING = 'SET_USER_FOLLOWING';
export const GET_USER_OFFER = 'GET_USER_OFFER';
export const SET_USER_OFFER = 'SET_USER_OFFER';
export const GET_INFO_USER = 'GET_INFO_USER';
export const SET_INFO_USER = 'SET_INFO_USER';
export const GET_PROFILE_USER = 'GET_PROFILE_USER';
export const SET_PROFILE_USER = 'SET_PROFILE_USER';
export const FOLLOW = 'FOLLOW';
export const SET_ALERT_MESSAGE = 'SET_ALERT_MESSAGE';
export const SET_USER_FOLLOW = 'SET_USER_FOLLOW';
export const SET_PROFILE = 'SET_PROFILE';
export const GET_USER_FRIEND = 'GET_USER_FRIEND';
export const SET_USER_FRIEND = 'SET_USER_FRIEND';
export const GET_NOTIFICATIONS = 'GET_NOTIFICATIONS';
export const SET_NOTIFICATIONS = 'SET_NOTIFICATIONS';
export const UPDATE_NOTIFICATIONS = 'UPDATE_NOTIFICATIONS';
export const SET_NOTIFICATIONS_AFTER_UPDATE = 'SET_NOTIFICATIONS_AFTER_UPDATE';
export const SET_NOTIFICATION = 'SET_NOTIFICATION';
export const SET_NOTIFICATIONS_FOLLOW = 'SET_NOTIFICATIONS_FOLLOW';
export const GET_NOTIFICATIONS_MESSAGES = 'GET_NOTIFICATIONS_MESSAGES';
export const SET_NOTIFICATIONS_MESSAGES = 'SET_NOTIFICATIONS_MESSAGES';
export const UPDATE_NOTIFICATIONS_MESSAGE = 'UPDATE_NOTIFICATIONS_MESSAGE';
export const SET_NOTIFICATIONS_MESSAGE_AFTER_UPDATE = 'SET_NOTIFICATIONS_MESSAGE_AFTER_UPDATE';
export const SET_NOTIFICATION_MESSAGE = 'SET_NOTIFICATION_MESSAGE';
